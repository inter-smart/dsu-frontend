...
export default function BscCurriculum({ data }) {
    if (!data) return null;
    ...
    return (
        <section className="relative w-full py-[40px] xl:py-[60px] 2xl:py-[65px] 3xl:py-[70px]">
            <div className="container">
                <div className="mb-[30px] xl:mb-[40px]">
                    <h2 className="cmn_Title mb-[10px] xl:mb-[15px]">{data.heading}</h2>
                    <p className="text_1 text-[#4A5565] dark:text-gray-300">{data.description}</p>
                </div>

                {/* Swiper Tab Slider with Nav Buttons */}
                <div className={`flex items-center gap-[8px] sm:gap-[12px] mb-[30px] xl:mb-[40px] max-w-full ${canSlide ? "w-full" : "w-fit"}`}>
                    {/* Prev Arrow */}
                    <button
                        ref={prevRef}
                        type="button"
                        aria-label="Previous tab"
                        className={`curriculum-tab-prev w-[32px] h-[32px] sm:w-[36px] sm:h-[36px] xl:w-[42px] xl:h-[42px] rounded-full bg-white dark:bg-white/10 border border-[#2121211a] dark:border-white/20 shadow-[0_2px_10px_rgba(0,0,0,0.06)] items-center justify-center text-[#212121] dark:text-white hover:bg-[#F97316] hover:text-white hover:border-[#F97316] transition-all cursor-pointer shrink-0 z-10 disabled:opacity-30 disabled:cursor-not-allowed disabled:hover:bg-white disabled:hover:text-[#212121] disabled:hover:border-[#2121211a] [&.swiper-button-lock]:!hidden ${!canSlide ? "!hidden" : "flex"}`}
                    >
                            ...
                            {data.semesters?.map((item, idx) => (
                                <SwiperSlide key={item.id} className="!w-auto">
                                    <button
                                        ...
                                        className={`h-[35px] xl:h-[40px] px-[20px] rounded-[30px] border text-[14px] xl:text-[16px] font-medium cursor-pointer transition-all duration-300 whitespace-nowrap
                                            ${activeIndex === idx
                                                ? "bg-gradient-to-r from-[#DC2626] to-[#F97316] text-white border-transparent"
                                                : "bg-white dark:bg-transparent border-[#DC2626]/30 dark:border-white/20 text-black dark:text-white hover:border-[#DC2626]"
                                            }`}
                                    >
                                        {item.label}
                                    </button>
                    ...
                    <button
                        ...
                        type="button"
                        aria-label="Next tab"
                        className={`curriculum-tab-next w-[32px] h-[32px] sm:w-[36px] sm:h-[36px] xl:w-[42px] xl:h-[42px] rounded-full bg-white dark:bg-white/10 border border-[#2121211a] dark:border-white/20 shadow-[0_2px_10px_rgba(0,0,0,0.06)] items-center justify-center text-[#212121] dark:text-white hover:bg-[#F97316] hover:text-white hover:border-[#F97316] transition-all cursor-pointer shrink-0 z-10 disabled:opacity-30 disabled:cursor-not-allowed disabled:hover:bg-white disabled:hover:text-[#212121] disabled:hover:border-[#2121211a] [&.swiper-button-lock]:!hidden ${!canSlide ? "!hidden" : "flex"}`}
                    >
                        <svg className="w-[14px] h-[14px] sm:w-[16px] sm:h-[16px]" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
                ...
                {data.semesters?.map((item, idx) => (
                    activeIndex === idx && (
                        <div key={item.id}>
                            <div className="border border-gray-100 dark:border-white/20 p-[20px] xl:p-[25px_20px] bg-white dark:bg-white/5">
