...
export default function AiAcademicMenubar({
  title = "Academic Menu",
  className = "",
  data,
}) {
  ...
  const navigationItems =
    data?.length > 0
      ? data.map(({ label, slug }) => ({ label, href: slug }))
      : menuItems;
  ...
  const isCurrentItemActive = (href) => {
    ...
    if (
      ...
      navigationItems.some(
        (m) => m.href.includes("#") && m.href.split("#")[0] === href,
      )
    ) {
      return false;
    }
          ...
          >
            <div className="w-[16px] flex items-center">
              <svg width="21" height="16" viewBox="0 0 21 16" fill="none">
                <path
                  d="M19.3 1.5871H0.7C0.3 1.5871 0 1.28952 0 0.793548C0 0.29758 0.3 0 0.8 0H19.3C19.7 0 20.1 0.29758 20.1 0.793548C20.1 1.28952 19.7 1.5871 19.3 1.5871Z"
                  fill="white"
                />
                <path
                  d="M19.3 8.53064H0.7C0.3 8.53064 0 8.23306 0 7.73709C0 7.24113 0.3 6.94355 0.8 6.94355H19.3C19.7 6.94355 20.1 7.24113 20.1 7.73709C20.1 8.23306 19.7 8.53064 19.3 8.53064Z"
                  fill="white"
                />
                <path
                  d="M19.3 15.4742H0.7C0.3 15.4742 0 15.1766 0 14.6806C0 14.1847 0.3 13.8871 0.8 13.8871H19.3C19.7 13.8871 20.1 14.1847 20.1 14.6806C20.1 15.1766 19.7 15.4742 19.3 15.4742Z"
                  fill="white"
                />
              </svg>
            </div>
                    ...
                    <Accordion type="single" collapsible>
                      ...
                      >
                        <div className="flex items-center justify-between p-[11px_15px]">
                          ...
                          >
                            <div
                              className={`text_1 font-semibold transition-colors duration-300 group-hover:text-[#F97316] text-[13px] ${
                                isActive
                                  ? "text-[#F97316]"
                                  : "text-[#212121] dark:text-[#F9FAFB]"
                              }`}
                            >
                              {item.label}
                            </div>
                        ...
                        <AccordionContent className="pb-[11px] px-[15px]">
                          <ul>
                            {item.submenu.map((subItem, subIdx) => {
                              const isSubActive = isCurrentItemActive(
                                subItem.href,
                              );
                              return (
                                  ...
                                  >
                                    <span
                                      className={`w-[4px] h-[4px] rounded-full shrink-0 ${
                                        isSubActive
                                          ? "bg-[#F97316]"
                                          : "bg-[#212121]/40 dark:bg-white/40"
                                      }`}
                                    />
                                    <span
                                      className={`text_1 text-[12px] transition-colors duration-300 hover:text-[#F97316] ${
                                        isSubActive
                                          ? "text-[#F97316] font-semibold"
                                          : "text-[#212121]/80 dark:text-[#F9FAFB]/80"
                                      }`}
                                    >
                                      {subItem.label}
                                    </span>
