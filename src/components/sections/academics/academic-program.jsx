            ...
            <div className="container">
                <div className="mb-[15px] lg:mb-[25px] 2xl:mb-[40px] 3xl:mb-[45px] flex max-lg:flex-wrap">
                    <div className="w-auto lg:flex-grow-1">
                        <div className="cmn_Title mb-[20px] 2xl:mb-[25px] 3xl:mb-[30px]">{data.heading}</div>
                        <div className="text_1 leading-[1.2] text-[#4A5565] lg:max-w-[90%]">
                ...
                <div className={`flex gap-[30px] lg:gap-[40px] xl:gap-[60px] ${variant === "aiprogram" ? "w-full flex-wrap" : "max-sm:flex-wrap gap-[15px] lg:max-w-[90%] w-full sm:w-1/2"}`}>
                    {data.categories.map((item, id) => (
                        <div
                            key={id}
                            className={`${variant === "aiprogram" ? "w-full pb-[35px] lg:pb-[45px] xl:pb-[65px] last-of-type:pb-0 not-last-of-type:border-b border-black/10" : "w-full sm:w-1/2"}`}
                        >
                            ...
                            {item.description && (
                                <p className="lg:max-w-[98%]">{item.description}</p>
                            )}
                            <div className="flex flex-wrap mt-[30px]">
                                {item.programs.map((program, idx) => (
                                    <div className={`${variant === "aiprogram" ? "w-full md:w-1/2" : "w-full"}`}>
                                        <Link
                                            href={program?.url}
                                            key={idx}
                                            className="group flex items-center justify-between font-medium py-[15px] 2xl:py-[18px] 3xl:py-[22px] lg:max-w-[80%]  border-b border-black/20 px-0 transition-colors duration-300"
                                        >
