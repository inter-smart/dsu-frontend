...
export default function WhyusAQAR({ data }) {
    const defaultTab = data.yearFilters[0]?.slug;

    return (
        <section className="relative py-[40px] xl:py-[60px] 2xl:py-[80px] 3xl:py-[120px]">
            <div className="container">
                <div className="mb-[45px]">
                    <h2 className="cmn_Title">{data.heading}</h2>
                    <p className="text_1">{data.description}</p>
                </div>
                <Tabs defaultValue={defaultTab} className="w-full">
                    <TabsList className="gap-[8px] bg-transparent p-0 mb-[25px] xl:mb-[40px]">
                        {data.yearFilters.map((item) => (
                            <TabsTrigger
                                key={item.id}
                                value={item.slug}
                                className="group min-w-[60px] lg:min-w-[115px] xl:min-w-[135px] 2xl:min-w-[150px] 3xl:min-w-[170px] h-[35px] 2xl:h-[40px] 3xl:h-[46px] flex items-center justify-center rounded-[5px] border-0
                                 bg-gradient-to-r from-[#DC2626] to-[#F97316] p-[1px] text-[13px] lg:text-[14px] xl:text-[16px] 2xl:text-[18px] 3xl:text-[20px] 
                                 w-fit overflow-hidden !no-underline transition-all duration-200 ease-in-out
                                ...
                                <div className="flex flex-col">
                                    {item.documents.map((doc) => (
                                        ...
                                        <Link
                                            key={doc.id}
                                            href={doc.file.url}
                                            className="group flex items-center justify-between py-[15px] 2xl:py-[18px] 3xl:py-[22px] border-b border-black/20 transition-colors duration-300  sm:max-w-[50%] lg:max-w-[30%]"
                                        >
                                            <div className="flex items-center justify-between gap-[15px]">
                                                ...
                                                <div className="text-[12px] xl:text-[14px] 2xl:text-[16px] 3xl:text-[20px] text-black"> {doc.label}</div>
